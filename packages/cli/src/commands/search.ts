...
export function registerSearchCommand(program: commander.Command, execute: ExecuteCommand) {
  program
    .command("search <query>")
    .alias("find")
    .description("按名称、分类和文档全文搜索")
    .option("-l, --limit <number>", "最多返回多少条结果", "10")
    .action((query: string, options: SearchCommandOptions, command: commander.Command) => {
